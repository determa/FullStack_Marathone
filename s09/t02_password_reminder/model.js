...
const transporter = nodemailer.createTransport({
    ...
    port: 587,
    auth: {
        user: 'alwju3rgykm3fwef@ethereal.email',
        pass: 'ubdqMb8ZxGnZCDUVwh'
    }
});
...
module.exports = class Model {
    ...
    async find_login() {
        return await pool.execute('SELECT * FROM heroes WHERE login = ?;', [this.login])
            .then(([rows]) => {
    ...
    async pass_find() {
        return await pool.query(`SELECT * FROM users WHERE email = ?;`, this.email)
            .then(async ([rows]) => {
                if (rows[0]) {
